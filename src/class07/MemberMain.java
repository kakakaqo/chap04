 ...
 * 
 * split된 배열에 들어 있는  값들로 회원 ㅐㄱ체 생성해서 회원 객체 전용 배열에 저장
 */
...
public class MemberMain {
	...
	// 20대 후반의 방문횟수 구하는 메소드
	private static int getVisitCount(Member[] members) {
		int me = 0;
		for(int i = 0; i < members.length; i++) {
			if( members[i].getAge().equals("20대후반") && members[i].getGender() == '여' ) {
				me = me + members[i].getNumVisit();
			}
		}
		return me; // 방문횟수 반환
	}
	...
	private static void printMember(Member[] members) {
		System.out.println("회원데이터");
		System.out.println("===============================================================");
		for (Member me : members) {
			System.out.println(me.getName() + "\t" + me.getJoinDate() + 
					"\t" + me.getLastVisitDate() + "\t" + me.getNumVisit() + 
					"\t" + me.getGender() + "\t" + me.getAge());
		}
		System.out.println("===============================================================");
	}
	...
	private static void initData(Member[] members) {
		...
		// 이 데이터를 split으로 잘라서 Member 객체를 사용해서
		// Member[] 배열에 저장하세요.
		...
		// 콤머(,)를 기준으로 자르면 -> "소영" "20180419" "20180615" "44" "여" "20대"
		for(int i = 0; i < member.length; i++) {
			String[] arr = member[i].split(","); // 콤머로 구분해서 자르고 배열에
			//arr[0] = "봉기", arr[1] = "20180419", arr[2] = "20181002",
			//arr[3] = 71, arr[4] = '남', arr[5] = "20대후반" 으로 나누어짐
			int numVisit = Integer.parseInt(arr[3]); // 문자열을 정수형으로 변경  "71" -> 71
			char gender = arr[4].charAt(0); // 문자열을 캐릭터형으로 변경 "남" -> '남'
			members[i] = new Member(arr[0], arr[1], arr[2],
					numVisit, gender, arr[5]); // 객체 생성
		}
	}
